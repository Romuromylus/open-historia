...
const Toolbar = memo(({ onOpenAdvisor, activePanel, onTogglePanel, forcesOpen = false, onToggleForces }) => {
    ...
    const eventsOpen = activePanel === "history";
    return (
        <div style={{ position: "fixed", bottom: "0.5rem", left: "0.5rem", height: "4rem", width: "16.5rem", gap: "0.75rem", padding: "0 0.1rem", backgroundColor: "rgba(17,24,39,0.9)", backdropFilter: "blur(4px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontFamily: "sans-serif", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 24px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.05)" }}>
        ...
        {/* Events — reopens the turn-events panel (also reachable via the « chevron
            on the date widget, which players kept missing after closing the panel). */}
        <button title="Events" style={{ width: "3.3rem", height: "3.3rem", borderRadius: "10px", border: hoveredEvents ? "1px solid rgba(255,255,255,0.2)" : eventsOpen ? "1px solid rgba(139,92,246,0.5)" : "1px solid rgba(255,255,255,0.1)", background: eventsOpen ? "linear-gradient(145deg,rgba(109,40,217,0.4),rgba(76,29,149,0.4))" : hoveredEvents ? "linear-gradient(145deg,rgba(40,55,80,0.95),rgba(20,30,50,0.95))" : "linear-gradient(145deg,rgba(30,42,65,0.95),rgba(15,22,40,0.95))", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.12s ease", boxShadow: hoveredEvents ? "inset 0 1px 0 rgba(255,255,255,0.1),0 2px 8px rgba(0,0,0,0.4)" : "inset 0 1px 0 rgba(255,255,255,0.06),inset 0 -1px 0 rgba(0,0,0,0.3),0 2px 6px rgba(0,0,0,0.35)", fontSize: "1.2rem", outline: "none", transform: hoveredEvents ? "translateY(-1px)" : "translateY(0)", color: "white", fontFamily: "sans-serif", flexShrink: 0 }}
        onMouseEnter={() => setHoveredEvents(true)} onMouseLeave={() => setHoveredEvents(false)}
        onClick={() => onTogglePanel("history")}>🗞️</button>
